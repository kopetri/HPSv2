...
class ViewModel(nn.Module):
    def __init__(self, latent_size, mlp_layers, dropout, sigmoid) -> None:
        super().__init__()
        self.encoder = EncoderSimple(img_size=224, latent_size=latent_size)
        self.decoder = Decoder(latent_size=latent_size, dropout=dropout, n_features=mlp_layers, apply_sigmoid=sigmoid)
